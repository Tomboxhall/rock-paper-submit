...
let playRound = (player1Choice) => {
  ...
  if (input.files && input.files[0]) {
    ...
    reader.onload = function (e) {
      // Assuming you have an object with image and name properties
      const selectedChoice = { image: e.target.result, name: "Selected Image" };
      showChoiceAndHighlight(selectedChoice, playerShowArea);
    };

    reader.readAsDataURL(input.files[0]);
  } else {
    showChoiceAndHighlight(arr[player1Index], playerShowArea);
  }

  showChoiceAndHighlight(arr[computerIndex], computerShowArea);
...
let showChoiceAndHighlight = (choice, showArea) => {
  showPlayerChoice(choice, showArea);
};
